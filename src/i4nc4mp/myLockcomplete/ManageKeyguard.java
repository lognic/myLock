...
// I added some documentation to certain parts to help explain the
// keyguardmanager facet of android.
...
public class ManageKeyguard
{
	...
	// this only can be used after we have paused/hidden the lockscreen with a
		...
		if (inKeyguardRestrictedInputMode())
		{
			...
			myKM.exitKeyguardSecurely(new OnKeyguardExitResult()
			{
				public void onKeyguardExitResult(boolean success)
				{
					...
					// without this call, all future disable calls will be
					// blocked
					// for not following the lockscreen rules
					// in other words reenable immediately restores a paused
					// lockscreen
					// but only queues restore for next screen off if a secure
					// exit has been done already
					if (success)
					{
