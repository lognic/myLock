...
// supports advanced power save for setting locked-down buttons (screen stays
// off)
...
public class CustomLockService extends MediatorService
{
	...
	// Flagged true upon Lock Activity exit callback, remains true until
	// StartLock intent is fired.
	
	public boolean PendingLock = false;
	// Flagged true upon sleep, remains true until StartLock sends first
	// callback indicating Create success.
	
	public boolean idle = false;
	// when the idle alarm intent comes in we set this true to properly start
	// closing down
	...
	public void onRestartCommand()
	{
		...
		/*
		 * ========Settings change re-start commands that come from settings
		 * activity
		 */
		
		if (persistent != fgpref)
		{// user changed pref
			if (persistent)
			{
		...
		{
			/*
			 * ========Safety start that ensures the settings activity toggle
			 * button can work, first press to start, 2nd press to stop
			 */
			Log.v("toggle request",
					"user first press of toggle after a startup at boot");
		}
	}
	...
	public void onFirstStart()
	{
		...
		if (persistent)
			doFGstart();
		// else send a toast telling user what mode is starting and whether stay
		// awake is active
		...
		// if not always holding partial we would only acquire at Lock activity
		// exit callback
		// we found we always need it to ensure key events will not occasionally
		// drop on the floor from idle state wakeup
		
		IntentFilter idleFinish = new IntentFilter(
				"i4nc4mp.myLockcomplete.lifecycle.IDLE_TIMEOUT");
		registerReceiver(idleExit, idleFinish);
		
		IntentFilter lockStart = new IntentFilter(
				"i4nc4mp.myLockcomplete.lifecycle.LOCKSCREEN_PRIMED");
		...
		// ("i4nc4mp.myLockcomplete.lifecycle.HOMEKEY_UNLOCK");
	...
	BroadcastReceiver lockStarted = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			// update the stored value only if it has changed, and is at least
			// 15
			// FIXME need to store the value in our prefs file
			
			try
			{
				newtimeout = android.provider.Settings.System.getInt(
						getContentResolver(),
			...
			if (newtimeout != 1)
				timeoutpref = newtimeout;
			else
				Log.v("lock start callback",
						"the system timeout is already 1, our stored pref is "
								+ timeoutpref);
			// for now this code protects us from improperly overwriting our
			// stored timeout pref with the reduced pref
			...
			// this alarm kills off the lock activity and this service, restores
			// KG, & starts the user present service
			
			// TODO we're going to want to start at stop it within the activity
			// wakeup as well
	...
	BroadcastReceiver lockStopped = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			if (!idle)
			{
				...
				// FIXME
				// this should restore screen off to our known user-pref that is
				// stored in prefs
				// that pref will be set at Lock start, when we detect a change
				// that isn't to 0 or 1
			...
			else
			{
				...
				// funny - you will see the regular lockscreen after this call
				// because it is restoring it from time that pattern was off
				// if you slide that, you land at the security pattern screen ;]
				// otherwise if it sleeps like that, next wakeup places us at
				// pattern screen
				
				Intent u = new Intent();
				u.setClassName("i4nc4mp.myLockcomplete",
						"i4nc4mp.myLockcomplete.UserPresentService");
				// service that reacts to the completion of the keyguard to
				// start this mediator again
				startService(u);
				stopSelf();
			}
		}
	 ...
	 * (!intent
	 * .getAction().equals("i4nc4mp.myLockcomplete.lifecycle.HOMEKEY_UNLOCK"))
	 * return;
	 * 
	 * ManageKeyguard.disableKeyguard(context); StartDismiss(context); return;
	 * 
	 * }};
	 */
	...
	BroadcastReceiver idleExit = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			Log.v("mediator idle reaction",
					"preparing to restore KG. timeout pref is " + timeoutpref);
			
			android.provider.Settings.System.putInt(getContentResolver(),
					android.provider.Settings.System.SCREEN_OFF_TIMEOUT,
					timeoutpref);
			
			// the idle flag will cause proper handling on receipt of the exit
			// callback from lockscreen
			// we basically unlock as if user requested, but then force KG back
			// on in the callback reaction
		}
	};
	...
	class Task implements Runnable
	{
		public void run()
		{
			...
			if (!PendingLock)
				return;// ensures break the attempt cycle if user has aborted
						// the lock
			// user can abort Power key lock by another power key, or timeout
			// sleep by any key wakeup
			...
			if (ManageKeyguard.inKeyguardRestrictedInputMode())
			{
				...
				// the keyguard exists here on first try if this isn't a timeout
				// lock
				shouldLock = false;
	...
	public void onScreenWakeup()
	{
		if (!PendingLock)
			return;
		// we only handle this when we get a screen on that's happening while we
		// are waiting for a lockscreen start callback
		
		// This case comes in two scenarios
		// Known bug (seems to be fixed)--- the start of LockActivity was
		// delayed to screen on due to CPU load
		// User aborting a timeout sleep by any key input before 5 second limit
		
		PendingLock = false;
		if (!shouldLock)
		{
			// this is the case that the lockscreen still hasn't sent us a start
			// callback at time of this screen on
			shouldLock = true;
		}
	...
	public void onScreenSleep()
	{
		...
		{
			Log.v("mediator screen off",
					"call flag in progress, aborting handling");
		...
		{
			PendingLock = true;
			// means trying to start lock (waiting for start callback from
			// activity)
			
			// FIXME stay awake mode is probably preventing the logic in the
			// quiet wake that flags that off
			// ---need to move more logic into the timeout task we have running
			// as a screen off will never come in to flag cpuwake off
	...
	private void StartLock(Context context)
	{
		...
		// this just calls a temporary KG pause. doing this allows us to be
		// recognized when we later want to re-enable
		...
		// without this flag my alarm clock only buzzes once. but with it wakeup
		// doesn't want to happen (also impacts handcent notifs)
		
		// | Intent.FLAG_ACTIVITY_NO_HISTORY
		// this flag will tell OS to always finish the activity when user leaves
		// it
		// when this was on, it was exiting every time it got created.
		// interesting unexpected behavior
		// even happening when i wait 4 seconds to create it.
		// | Intent.FLAG_ACTIVITY_NO_ANIMATION)
		// because we don't need to animate... O_o doesn't really seem to be for
		// this
	...
	{// try the alpha keyguard manager secure exit
		...
		// because we're already showing on top of keyguard this gets the job
		// done
	...
	public void StartDismiss(Context context)
	{
		...
		dismiss.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK// For some reason it
														// requires this even
														// though we're already
														// an activity
				| Intent.FLAG_ACTIVITY_NO_USER_ACTION// Just helps avoid
														// conflicting with
														// other important
														// notifications
				| Intent.FLAG_ACTIVITY_NO_HISTORY// Ensures the activity WILL be
	...
	public void onCallStart()
	{
		...
		if (!shouldLock)
		{
			...
			// we don't have to do anything for calls initiated as no lock
			// activity is alive
			Intent intent = new Intent(
					"i4nc4mp.myLockcomplete.lifecycle.CALL_START");
	...
	public void onCallEnd()
	{
		...
		// therefore, all calls ending while screen is off result in restart
		// lockactivity
		// if screen is awake we check for KG, exit if needed, and reset
		// shouldLock to true
		...
		if (IsAwake())
		{
			...
			if (ManageKeyguard.inKeyguardRestrictedInputMode())
				DoExit(mCon);
			// TODO change this to the dismiss activity now that we have timing
			// bugs fixed
		}
		else
		{
	...
	{
		Intent intent = new Intent(
				"i4nc4mp.myLockcomplete.lifecycle.CALL_PENDING");
		getApplicationContext().sendBroadcast(intent);
		// lets the activity know it should not treat focus loss as a navigation
		// exit
